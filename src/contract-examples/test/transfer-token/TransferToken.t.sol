...
contract TransferTokenTestUnit is DSTestPlus {
  ...
  function setUp() public {
    ...
    transfer = new TransferToken(connext);

    vm.label(address(connext), "Connext");
    vm.label(address(transfer), "TransferToken");
  ...
  function test_TransferToken_transferShouldTransferTokensFromCaller(uint256 amount) public {
    ...
    // userChainA must approve transfer to TransferToken contract
    vm.prank(userChainA);
    ...
    // Test that tokens are sent from userChainA to TransferToken contract
    vm.expectCall(
      address(token), 
      abi.encodeCall(
        IERC20.transferFrom, 
...
/**
 * @title TransferTokenTestForked
 * @notice Integration tests for TransferToken. Should be run with forked testnet.
 */
contract TransferTokenTestForked is DSTestPlus {
  ...
  function test_TransferToken_transferShouldCallXCall(uint256 amount) public {
